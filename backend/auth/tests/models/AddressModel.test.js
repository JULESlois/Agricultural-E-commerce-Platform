...
describe('AddressModel', () => {
  ...
  describe('create', () => {
    ...
    it('应该在错误时回滚事务', async () => {
      ...
      await expect(AddressModel.create(1, addressData)).rejects.toThrow('Database error');
  ...
  describe('findByUserId', () => {
    it('应该返回用户地址列表', async () => {
      ...
      expect(pool.query).toHaveBeenCalledWith(
        expect.stringContaining('SELECT address_id, address_name, receiver'),
        [1]
      );
  ...
  describe('update', () => {
    it('应该更新地址信息', async () => {
      ...
      expect(pool.query).toHaveBeenCalledWith(
        ...
        expect.arrayContaining([1, 101, '王小五', '13700137001'])
      );
      expect(result).toEqual({ address_id: 101 });
    });
  });
