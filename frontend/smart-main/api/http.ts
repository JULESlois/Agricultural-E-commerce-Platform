const getBase = (key: string, fallback: string) => {
  ...
  return typeof v === 'string' && v.length > 0 ? v.trim() : fallback;
};

export const BASES = {
  MARKET: getBase('VITE_MARKET_BASE', 'http://localhost:3010/api'),
...
export async function request(path: string, options: RequestInit = {}, base?: string) {
  ...
  try {
    ...
    if (!resp.ok) {
      ...
      throw { code, httpStatus: resp.status, message, detail: data };
    }
    return data;
  } catch (e: any) {
    if (e?.code) throw e;
    const offline = typeof navigator !== 'undefined' && !navigator.onLine;
    const code = offline ? 'network.offline' : 'unknown';
    const message = formatMessage(code);
    throw { code, message };
  }
}
...
import { formatMessage, mapHttpStatusToCode } from '../utils/errors';
